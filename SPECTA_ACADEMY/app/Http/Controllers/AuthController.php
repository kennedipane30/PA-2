...
class AuthController extends Controller {
    ...
    /**
     * 8. AMBIL DAFTAR SOAL (MODIFIKASI: UNTUK MULAI QUIZ)
     */
    public function getQuestions(Request $request): JsonResponse {
        $questions = Question::where('tryout_id', $request->tryout_id)->get();
        if ($questions->isEmpty()) {
            return response()->json(['status' => 'error', 'message' => 'Soal belum tersedia untuk tryout ini'], 404);
        }
        return response()->json(['status' => 'success', 'data' => $questions], 200);
    }

    /**
     * 9. SUBMIT TRYOUT & DOWNLOAD
     */
    public function submitTryout(Request $request): JsonResponse {
        ...
        return response()->json(['status' => 'success', 'score' => $score, 'resultID' => $result->resultsID]);
    }
