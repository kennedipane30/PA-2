...
class AuthController extends Controller {
    ...
    public function registerSiswa(Request $request): JsonResponse {
        ...
        try {
            $user = User::create([
                'name' => trim($request->name), 'email' => $request->email, 'phone' => $request->nomor_wa,
    ...
    public function updateProfile(Request $request): JsonResponse {
        $v = Validator::make($request->all(), [
            'parent_name' => 'required|string', 'alamat' => 'required|string', 'wa_ortu' => 'required', 'nisn' => 'required', 'dob' => 'required|date',
        ]);
        if ($v->fails()) return response()->json(['status' => 'error', 'message' => $v->errors()->first()], 422);
    ...
    public function getClassContent(Request $request): JsonResponse {
        ...
        return response()->json([
            ...
            'duration' => '30 Hari',
            'materi' => $materi,
            'tryouts' => $tryouts // Kirim data tryout
        ]);
    }

    // --- 7. LOGIKA JADWAL & LOGOUT ---
    ...
    /**
         * 8. LOGIKA TRYOUT (SUBMIT & PDF)
         */
        public function submitTryout(Request $request): JsonResponse
        {
            $userAnswers = $request->answers; // JSON: {"1":"A", "2":"C"}
            $correctCount = 0;

            // Ambil semua soal untuk tryout ini
            $questions = Question::where('tryout_id', $request->tryout_id)->get();

            foreach ($questions as $q) {
                // Cek apakah jawaban siswa cocok dengan kunci_jawaban di DB
                // Gunakan questionsID sesuai ERD
                if (isset($userAnswers[$q->questionsID]) && $userAnswers[$q->questionsID] == $q->correct_answer) {
                    $correctCount++;
                }
            }

            // Hitung skor (Integritas Data)
            $totalSoal = count($questions);
            $score = $totalSoal > 0 ? ($correctCount / $totalSoal) * 100 : 0;
            ...
            return response()->json([
                'status' => 'success',
                'score' => $score,
                'resultID' => $result->resultsID // Gunakan resultsID sesuai ERD
            ]);
        }

        /**
         * FUNGSI DOWNLOAD PDF PEMBAHASAN
         */
        public function downloadPembahasan($result_id)
        {
            // Ambil data hasil beserta soal dan jawabannya
            $result = TryoutResult::with(['tryout.questions'])->findOrFail($result_id);

            // Load view PDF (Kita akan buat filenya setelah ini)
