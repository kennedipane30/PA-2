...
<div class="container mx-auto p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <button onclick="document.getElementById('modalUpload').classList.remove('hidden')" class="bg-spekta text-white px-4 py-2 rounded-lg font-bold hover:bg-red-700 transition">
            + Upload Materi Baru
        </button>
    </div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 font-medium text-gray-800">{{ $m->judul }}</td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded text-xs font-bold {{ $m->tipe == 'pdf' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                            {{ strtoupper($m->tipe) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-gray-500">{{ $m->created_at->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-center flex justify-center gap-2">
                        <a href="{{ asset('storage/' . $m->file_path) }}" target="_blank" class="text-blue-500 hover:underline">Lihat</a>
                        <form action="{{ route('pengajar.materi.destroy', $m->id) }}" method="POST" onsubmit="return confirm('Hapus materi ini?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center text-gray-400">Belum ada materi yang diunggah.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="modalUpload" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-8 max-w-md w-full">
        <h2 class="text-xl font-bold mb-4">Form Upload Materi</h2>
        <form action="{{ route('pengajar.materi.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label class="block text-sm font-bold mb-2">Pilih File (PDF/Video)</label>
                <input type="file" name="file" class="w-full" required>
            </div>
            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('modalUpload').classList.add('hidden')" class="bg-gray-200 px-4 py-2 rounded-lg">Batal</button>
                <button type="submit" class="bg-spekta text-white px-4 py-2 rounded-lg font-bold">Simpan</button>
            </div>
        </form>
    </div>
</div>
